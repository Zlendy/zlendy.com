	...
	{#await loadData()}
		<div class="zy-shadow-loading" out:fade={{ duration: 250 }}>
			{#each { length: limit } as _, index}
				<BoardImage status={Status.Loading} {index} />
			{/each}
		</div>
	{:then images}
		<div class="zy-shadow-loaded" in:fade={{ delay: 251, duration: 250 }}>
			{#each images as image, index (image.id)}
				<BoardImage status={Status.Loaded} {index} {...image} />
			{/each}
		</div>
	{:catch _}
		<div class="zy-shadow-loaded" in:fade={{ delay: 251, duration: 250 }}>
...
<style lang="scss">
	...
	.zy-shadow-inner-container:global(:has(.zy-shadow-loading) .zy-shadow-loaded) {
